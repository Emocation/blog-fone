    ...
    <div class="login" id="login">
      ...
      <div class="login_row">
        <div class="login_user" style="position: relative">
          ...
          <input v-model="user.username" class="login_input"  placeholder="用户名" type="text" maxlength="16">
        </div>
        <div class="login_user" style="position: relative">
          <div class="icon"><img src="static/images/lock_icon.png"></div>
          <input v-model="user.password" class="login_input" placeholder="密码" type="password" maxlength="16">
        </div>
        <div class="login_user" style="position: relative">
          <div class="icon"><img src="static/images/user_icon.png"></div>
          <input v-model="user.email" class="login_input" placeholder="邮箱" type="text">
        </div>
          <el-button class="login_button" @click="Sign()" round>注册</el-button>
      </div>
    </div>
  </div>
</template>
<script>
  import {sign} from '../api/adminLogin'
  export default {
    name: "login",
    data() {
      return {
    ...
    methods: {
      ...
      Sign(){
        if(this.user.username===null||this.username===''){
          this.$message({
            message: '用户名不能为空',
            type: 'error'
          })
        }else if(this.user.password===null||this.password===''){
          this.$message({
            message: '密码不能为空',
            type: 'error'
          })
        }
        if(!this.sendEmail(this.user.email)){
            this.$message({
              message: '邮箱格式不正确',
               type: 'error'
            })
        }else{
          sign(this.user).then(res=> {
            console.log(res)
            if(res.code === 200){
              if(res.data){
